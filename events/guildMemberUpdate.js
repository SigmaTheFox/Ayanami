...
module.exports = async (ayanami, oldMember, newMember) => {
    if (!oldMember.isCommunicationDisabled() && newMember.isCommunicationDisabled()) {
        ...
        try {
            ...
            console.log(`${target.username || target.id || target.toString()} has the DMs blocked. Couldn't send mute message.`);
            ayanami.logger.log(`${target.username || target.id || target.toString()} has the DMs blocked. Couldn't send mute message.`);
        } finally {
