...
module.exports = {
    ...
    async execute(ayanami, interaction) {
        ...
        if (input.length > 1024) return interaction.reply({ content: "The text to translate can't be longer than 1024 characters.", ephemeral: true });

        try {
            let { text } = await deepl.translate(input, language);
            if (text.length > 1024) return interaction.reply({ content: "The translated text is longer than 1024 characters.\nYou might want to use https://translate.google.com", ephemeral: true });
            ...
            return interaction.reply({ content: "There was an error while translating. Try again later.", ephemeral: true });
        }
    }
}
