...
async function sendEmbed(interaction, user) {
    ...
    try {
        ...
        if (!osuUser.name) return interaction.reply({ content: `The user **${user}** was not found`, ephemeral: true });
        ...
        return message.reply({ content: "There was an error using this command", ephemeral: true });
    }
}
...
module.exports = {
            ...
            cmd.setName("get")
                ...
                .addStringOption(opt =>
                    opt.setName("username")
                        .setDescription("The osu! username")))
        .addSubcommand(cmd =>
            cmd.setName("set")
    ...
    async execute(ayanami, interaction) {
        ...
        if (interaction.options.getSubcommand() === "get") {
            ...
            if (!os && !username) return interaction.reply(`Please specify a username or add yours with the **/osu set** command.`)
            if (!username) return sendEmbed(interaction, os.get("osu_id"));
            else return sendEmbed(interaction, username);
        }
        else if (interaction.options.getSubcommand() === "set") {
            let result = await OsuDB.findOrCreate({ where: { discord_id: discordUser.id }, defaults: { osu_id: username } });
            let created = result[1];

            if (!created) {
                OsuDB.update({ osu_id: username }, { where: { discord_id: discordUser.id } });
                return interaction.reply({ content: `You set **${username}** as your default osu! username`, ephemeral: true });
            }
            return interaction.reply({ content: `You set **${username}** as your default osu! username`, ephemeral: true })
        }
    }
