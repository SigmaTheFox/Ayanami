...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('birthday')
    .setDescription(
      "Set or remove your birthday to Ayanami's reminder, or get someone else's"
    )
    .addSubcommand((cmd) =>
      cmd
        .setName('set')
    ...
    .addSubcommand((cmd) =>
      cmd
        .setName('remove')
        .setDescription('Remove your birthday from the reminder')
    )
    .addSubcommand((cmd) =>
      cmd
        .setName('get')
  ...
  async execute(ayanami, interaction) {
    if (interaction.options.getSubcommand() === 'set')
      return setBirthday(ayanami, interaction)
...
async function setBirthday(client, interaction) {
  ...
  if (isValidDate(day, month, 2020)) {
    ...
    console.log(
      `${interaction.user.username} set their birthday to ${month}-${day}`
    )
    client.logger.log(
      `${interaction.user.username} set their birthday to ${month}-${day}`
    )
...
async function removeBirthday(client, interaction) {
  ...
  if (result) {
    ...
    if (!destroyed)
      return interaction.reply({
        content:
          "You didn't set your birthday yet. use `/birthday set` to do so",
        ephemeral: true,
      })
...
async function getBirthday(interaction) {
  let user = interaction.options.getUser('user'),
    birthday = await BirthdaysDB.findOne({ where: { discord_id: user.id } })

  if (!birthday)
    return interaction.reply(
      `**${user.username}** doesn't have their birthday set.`
    )
  return interaction.reply(
    `**${user.username}'s** birthday is ${birthday.get('birthday')}`
  )
}
