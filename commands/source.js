...
async function getSource(client, imgURL) {
	if (!imgURL.includes("http://") && !imgURL.includes("https://")) return "Please enter a valid URL";
	...
	try {
		let results = await sagiriClient(imgURL, {
...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((cmd) =>
			cmd
				...
				.addStringOption((opt) =>
					opt.setName("link").setDescription("The direct URL to the image").setRequired(true)
				)
		)
		.addSubcommand((cmd) =>
			cmd
	...
	async execute(ayanami, interaction) {
		if (interaction.options.getSubcommand() === "url") {
			...
			if (typeof source === "string") return interaction.reply(source);
			else return sendEmbed(interaction, source, interaction.options.getAttachment("image").url);
		}
	},
};
