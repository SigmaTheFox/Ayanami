	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((cmd) =>
			cmd
				...
				.addUserOption((opt) =>
					opt.setName("user").setDescription("The offending user to report").setRequired(true)
				)
			...
			cmd
				...
				.addStringOption((opt) =>
					opt
						.setName("message")
						.setDescription(
							"The link of the message to report (right click message > Copy Message Link )"
						)
						.setRequired(true)
				)
		),
	...
	async execute(ayanami, interaction) {
		...
		if (interaction.options.getSubcommand() === "dm") {
			...
			let embed = new EmbedBuilder()
				...
				.setTitle("DM report")
				.setDescription(
					`**${reporter.tag}** reported **${offender.tag} (${offender.id})** for sending inappropriate messages in DM *Check Screenshot*`
				)
				.setImage(screenshot)
				.setColor("Aqua");

			report_channel.send({
				content: `${mod_role} there is a new report.`,
				embeds: [embed],
			});
			interaction.reply({
			...
			if (
				!/https?:\/\/(\w.*\.)?discord\.com\/channels\/[0-9].*\/[0-9].*\/[0-9].*\/?/gi.test(messageURL)
			)
