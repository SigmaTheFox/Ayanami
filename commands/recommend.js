...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption(opt =>
			opt
				...
				.setRequired(true)
				.addChoices({ name: 'server', value: 's' }, { name: 'render', value: 'r' })
		)
		.addStringOption(opt =>
			opt
				.setName('anonymity')
				...
				.setRequired(true)
				.addChoices({ name: 'visible', value: 'v' }, { name: 'anonymous', value: 'a' })
		)
		.addStringOption(opt =>
			opt.setName('recommendation').setDescription('Your recommendation').setRequired(true)
		)
		.addAttachmentOption(opt =>
			opt.setName('attachment').setDescription('An example attachment if needed')
		),
	/**
	...
	async execute(ayanami, interaction) {
		member = ayanami.guilds.cache
			.get('724873614104461322')
		...
		embed = new EmbedBuilder().setTitle(typeTitle[type]).setColor(45055);
		...
		if (anonymity === 'v')
			...
			embed.setAuthor({
				name: interaction.user.username,
				iconURL:
					'https://cdn.discordapp.com/attachments/340814937435668480/783290091313037332/question-mark-768x768.png',
			});
		...
		interaction.reply({
			content: 'Successfully posted recommendation',
